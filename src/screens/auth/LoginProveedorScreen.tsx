...
export default function LoginProveedorScreen() {
  ...
  const [usuario, setUsuario] = useState('moliselva');
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await api.post('/authProveedor/login', { usuario, contraseña });
      // console.log(response);
      if (response.data.token) {
        signIn(response.data, 'proveedor');
        Toast.show({
          type: 'success',
          text1: 'Éxito',
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={{color: COLORS.white, textAlign: 'center', marginTop: SPACING.space_18}}>Versión 1.0</Text>
